...
function popupButtonClick(
  ...
  } else if (!collect && coinBag.length > 0) {
    if (coinBag.length > 0) {
...
function saveGame() {
  ...
  localStorage.setItem("CoinState", JSON.stringify(coinBag))
}

function restorGame() {
  removeMarkings(true, true);
  returnBoard(localStorage.getItem("BoardState")!);
  coinBag = JSON.parse(localStorage.getItem("CoinState")!);
  coinDisplay.innerHTML = "Coins: " + coinBag.length;
  makeMarkings();
  B.knownCache.forEach((cache) => {
